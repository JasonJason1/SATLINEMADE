...
class _PictureScreenState extends State<PictureScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Look who's open your box"),
      ),
      body: FutureBuilder(
          ...
          if (snapshot.connectionState == ConnectionState.done) {
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                String imageUrl = snapshot.data[snapshot.data.length - index - 1]['url'];
                String dateTimeTaken = snapshot.data[snapshot.data.length - index - 1]['dateTimeTaken'];
                return SizedBox(
                  height: 300,
                  width: 250,
                  child: PictureItem(
